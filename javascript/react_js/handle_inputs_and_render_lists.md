...
1. Instead of displaying unordered list items, keep track of the total number of tasks in state and for each task display its number according to its order in the tasks array. Yes, you could also do that with a simple ordered list, but where's the fun in that? Try using state.
3. Implement a delete button for each task. The delete button should remove the specific task from the state array. Don't forget to never directly assign state. If you want you can use [Font Awesome](https://fontawesome.com/) for the icon.
